...
# %% [markdown]
# In this example, we program the move layout employed in the demonstration of logical magic state distillation for a distance-5 color code, as presented in QuEra’s recent publication: **“Experimental demonstration of logical magic state distillation”, Nature, 2025**.
...
path_shift_dist = 3.0 # distance that is used to shift the atom path out of the columns or rows
...
def entangle_cols(ctrls: ilist.IList[int, Any], qargs: ilist.IList[int, Any]):
    ...
    zone = spec.get_static_trap(zone_id="traps") #fill the defined zone specification with traps
    traps_shape = grid.shape(zone) # get the shape of the trap array
    all_rows = ilist.range(traps_shape[1]) # generate an ilist from 0 to the number of rows in the trap array

    src = grid.sub_grid(zone, ctrls, all_rows) # subgrid of control qubits that are picked up
    dst = grid.sub_grid(zone, qargs, all_rows) # subgrid of target qubits that are entangled with the control qubits

    # define the moves
    first_waypoint = grid.shift(src, 0.0, -path_shift_dist) # shifting the src grid down along the y-axis

    second_waypoint =  grid.shift(dst, -entangling_pair_dist, -path_shift_dist) # shift the to the respective x-positions of the target qubits and add an offset in the x-direction

    third_waypoint =  grid.shift(dst, -entangling_pair_dist, 0.0) # shift the dst grid back up into the original y-position to form atom paris that will get a gate

    waypoints = ilist.IList([src, first_waypoint, second_waypoint, third_waypoint]) # combine the waypoints into an ilist
    reverse_waypoints = ilist.IList([third_waypoint, second_waypoint, first_waypoint, src]) # the revese waypoints defining the reverse path back to the original positions

    move_by_waypoints(waypoints, True, False) # move the qubits along the waypoints, True means that the atoms are picked up and False means that they are not dropped at the end of the move
    gate.top_hat_cz(zone) # apply the entangling gate to the atoms that are now in the right positions (paired up)
    move_by_waypoints(reverse_waypoints, False, True) # move the atoms back to their original positions, False means that the atoms are not picked up (since they are still in the AOD) and True means that they are dropped back into their static trap site at the end of the move
...
def entangle_rows(ctrls: ilist.IList[int, Any], qargs: ilist.IList[int, Any]):
    ...
    zone = spec.get_static_trap(zone_id="traps") #fill the defined zone specification with traps
    traps_shape = grid.shape(zone) # get the shape of the trap array
    all_rows = ilist.range(traps_shape[0]) # generate an ilist from 0 to the number of columns in the trap array

    src = grid.sub_grid(zone, all_rows, ctrls)
    dst = grid.sub_grid(zone, all_rows, qargs)

    # define the moves
    first_waypoint = grid.shift(src, entangling_pair_dist, 0.0) # shift the src grid to the right along the x-axis

    second_waypoint = grid.shift(dst, entangling_pair_dist, 0.0) # move the src grid to the y-positions of the target qubits

    waypoints = ilist.IList([src, first_waypoint, second_waypoint]) # combine the waypoints into an ilist
    reverse_waypoints = ilist.IList([second_waypoint, first_waypoint, src]) # the revese waypoints defining the reverse path back to the original positions
...
def get_final_positions(src, dst, offset: float):
    ...
    assert len(src) == len(dst), "Source and destination lists must be of the same length."
...
def entangle_cols_low_dist(ctrls: ilist.IList[int, Any], qargs: ilist.IList[int, Any]):
    ...
    last_x = get_final_positions(src_x, dst_x, entangling_pair_dist) # get the nearest final positions for the control qubits
...
def make_main():
    ...
    @move(typeinfer=True)
    def main():
        """Main move kernel function that defines a slightly modified move pattern for the logical magic state distillation experiment."""

        init.fill([spec.get_static_trap(zone_id="traps")])

        # encode logical qubits
        entangle_cols_low_dist([1, 10, 12, 13], [3, 7, 14, 16])
        entangle_cols_low_dist([4, 8, 11, 15], [7, 10, 14, 16])
        entangle_cols_low_dist([2, 8, 9, 10, 14], [4, 6, 7, 13, 16])
        entangle_cols_low_dist([0, 3, 5, 10, 11], [2, 6, 8, 12, 13])
        entangle_cols_low_dist([0, 2, 4, 6, 8, 12], [1, 3, 5, 7, 9, 15])

        # entangle the logical qubits
        entangle_rows([0, 2], [1, 3])
        entangle_rows([1, 3], [2, 4])
        entangle_rows([3, 4], [0, 1])
